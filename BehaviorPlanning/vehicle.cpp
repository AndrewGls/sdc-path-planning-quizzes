...
void Vehicle::update_state(map<int, vector < vector<int> > > predictions) {
	...
	*/
	...
	// Prediction is array of N predicted locations of vehicle like { lane, s } for N timesteps.

	if (state == "CS")
		state = "KS";

	// It needs to have list of vehicles to detect collisions correctly!!! I use simplest approach to do this.

	map<string, double> costs; // mapping state to cost function value.

	auto kl_cost = [this](int delta_lane) {
		auto dl = (lane + delta_lane - goal_lane);
		return dl * dl;
	};

	auto lc_cost = [this](int delta_lane, const map<int, vector<vector<int>>>& predictions) {
		for (auto it = predictions.begin(); it != predictions.end(); ++it) {
			if (it->first != -1) {
				if (lane == it->second[0][0] && abs(s - it->second[0][1]) <= L) {
					return 10;
				}
			}
		}
	...
	costs.insert(pair<string, double>("KL", kl_cost(0)));
	costs.insert(pair<string, double>("PLCL", kl_cost(1)));
	costs.insert(pair<string, double>("PLCR", kl_cost(-1)));
	costs.insert(pair<string, double>("LCL", lc_cost(1, predictions)));
	costs.insert(pair<string, double>("LCR", lc_cost(-1, predictions)));
	...
	auto itNext = next_states.end();
	auto itNext2 = next_states.end();
	for (auto it = next_states.begin(); it != next_states.end(); ++it) {
		auto res = costs.find(*it);
		if (res != costs.end()) {
			if (res->second < max_cost) {
				max_cost = res->second;
				itNext = it;
			}
			else if (res->second == max_cost) {
				itNext2 = it;
			}
		}
	}

	if (itNext != next_states.end()) {
		state = *itNext;
		if (itNext2 != next_states.end()) {
			if ((*itNext == "PLCL" || *itNext2 == "PLCL") && ((*itNext == "LCL" || *itNext2 == "LCL"))) {
				state = "LCL";
			}
			else if ((*itNext == "PLCR" || *itNext2 == "PLCR") && ((*itNext == "LCR" || *itNext2 == "LCR"))) {
				state = "LCR";
			}
		}
	}
